...
function! AddToVcsIgnore(toIgnore) abort
    if (filewritable('.gitignore') == 1)
        call writefile(["", "# generated by vim", a:toIgnore], ".gitignore", "a")
    endif
endfunction
...
function! ClearSession() abort
    let g:should_save_session = 0
    silent! exe "!rm .vim/session.vim > /dev/null 2>&1"
    echo ":!rm .vim/session.vim"
endfunction

function! StartKeepingSession() abort
    let g:should_save_session = 1
    echo "Using: mksession! .vim/session.vim"
endfunction
